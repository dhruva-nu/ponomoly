...
/** Card wrapper style for the sign-in panel. */
...
export default function NameGate({
  ...
  return (
    <div style={panelStyle}>
      <SignInHeader roomId={roomId} />
      <NameInput name={name} onChange={setNameInput} onSubmit={submit} />
      <JoinButton valid={valid} onClick={submit} />
    </div>
  );
}

/** Title block: kicker, room heading, and helper caption. */
function SignInHeader({ roomId }: { roomId: string }) {
  return (
...
/** The display-name text field; Enter submits. */
function NameInput({ name, onChange, onSubmit }: { name: string; onChange: (value: string) => void; onSubmit: () => void }) {
  return (
    <input
      autoFocus
      value={name}
...
/** Full-width join CTA, dimmed when the name is empty. */
function JoinButton({ valid, onClick }: { valid: boolean; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      disabled={!valid}
