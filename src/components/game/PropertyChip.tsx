...
type ChipInfo = {
  ...
  unmortgageable: boolean;
  hasActions: boolean;
  houseBadge: string;
  nextIsHotel: boolean;
};

/** Derive the chip's display state and which management actions are available. */
function useChipInfo(
  spaceIndex: number,
  state: GameState,
  ownerIndex: number,
  canBuild: boolean,
  ...
  return { space, accentColor, mortgaged, level, cost, buildable, sellable, mortgageable, unmortgageable, hasActions, houseBadge, nextIsHotel };
}

/** The always-visible header row: accent bar, name, badges, and a manage toggle. */
function ChipHeader({
  info,
  open,
  onToggle,
  ...
  const { space, accentColor, mortgaged, houseBadge, hasActions } = info;
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 7, minWidth: 0 }}>
      ...
      {hasActions && (
        <button onClick={onToggle} title="Manage property" style={{
...
/** The expanded action row: build / sell / mortgage / unmortgage buttons. */
function ChipActions({
  ...
  const { level, cost, buildable, sellable, mortgageable, unmortgageable, nextIsHotel } = info;
  return (
...
export default function PropertyChip({
  ...
  const info = useChipInfo(spaceIndex, state, ownerIndex, canBuild, canManage);
  const { mortgaged, hasActions } = info;
  ...
  return (
    <div style={{
      ...
      <ChipHeader info={info} open={open} onToggle={() => setOpen((value) => !value)} />
      {hasActions && open && (
        <ChipActions spaceIndex={spaceIndex} info={info} onBuild={onBuild} onManage={onManage} />
      )}
    </div>
  );
}
