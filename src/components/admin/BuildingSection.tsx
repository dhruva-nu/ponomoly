  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 6, marginTop: 7, flexWrap: "wrap" }}>
        <button onClick={() => run({ kind: "setBuildings", pos, level: Math.min(5, level + 1) })} style={panelButton("#2bd9a0")}>Build House +1</button>
        <button onClick={() => run({ kind: "setBuildings", pos, level: Math.max(0, level - 1) })} style={panelButton("#ffd23c")}>Remove −1</button>
        <button onClick={() => run({ kind: "setBuildings", pos, level: 5 })} style={panelButton("#ff8a3c")}>Hotel</button>
        <button onClick={() => run({ kind: "setBuildings", pos, level: 0 })} style={panelButton("#9fb4d8")}>Clear</button>
      </div>
    </div>
  );
}
